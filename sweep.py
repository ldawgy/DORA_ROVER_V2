...
def pattern_lawnmower(width_s=1.0, height_s=1.2, passes=5,
    ...
    for p in range(passes):
        print(f"🟩 Pass {p+1}/{passes}: moving {'forward' if direction==1 else 'backward'}")

        # Drive the lane
        drive(0, speed_pwm * direction, 0, height_s)
        stop()

        # Rotate 180° to face opposite direction (for next pass)
        if p != passes - 1:  # skip last one
            print("↪️ Turning 180°")
            drive(turn_pwm * direction, 0, 0, turn_time)
            stop()

            # Strafe sideways to next lane
            print("➡️ Shifting to next lane")
            drive(0, 0, strafe_pwm, width_s)
            stop()

        # Flip the driving direction for the next pass
        direction *= -1
